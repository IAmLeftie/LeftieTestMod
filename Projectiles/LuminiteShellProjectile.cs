...
{
    public class LuminiteShellProjectile : ModProjectile
    {
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Luminite Shell");
		...
		public override void Kill(int timeLeft)
		{
			...
			//Create a Vector2 holding the launch velocity of the new cluster projectiles
			Vector2 launchVelocity = new Vector2(-3f, 0);
			//Spawn 5 ShrapnelProjectiles splitting off at different angles tangent from the Shrapnel Ball at the moment of death
			for (int i = 0; i < 5; i++)
			{
				//Rotate it by pi/6
				launchVelocity = launchVelocity.RotatedBy(0.523598776f);

				//Create a new projectile
				Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, launchVelocity,
					ModContent.ProjectileType<MiniShellProjectile>(), Projectile.damage / 5, Projectile.knockBack / 3, Projectile.owner);
			}
		}
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
			//Create a Vector2 holding the launch velocity of the new cluster projectiles
			Vector2 launchVelocity = new Vector2(-3f, 0);
			//Spawn 3 ShrapnelProjectiles splitting off at different angles tangent from the Shrapnel Ball at the moment of death
			for (int i = 0; i < 3; i++)
			{
				//Rotate it by pi/4
				...
				//Create a new projectile
				Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Top, launchVelocity,
					ModContent.ProjectileType<MiniShellProjectile>(), Projectile.damage, Projectile.knockBack / 3, Projectile.owner);
			}
		}
